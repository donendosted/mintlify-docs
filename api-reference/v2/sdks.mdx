...
| `/databases/*` and `/databases` | `client.tenants` | Create, list, delete, status, collections, stats. Legacy `/tenants/*` paths remain as deprecated aliases. |
...
| Method | Endpoint |
|---|---|
| `client.tenants.create()` | `POST /databases` |
| `client.tenants.list()` | `GET /databases` |
| `client.tenants.delete()` | `DELETE /databases` |
| `client.tenants.status()` | `GET /databases/status` |
| `client.tenants.sub_tenants()` | `GET /databases/collections` |
| `client.tenants.stats()` | `GET /databases/stats` |
...
```
```python Python (Async)
response = await async_client.tenants.create(database="my_first_tenant")
```
```typescript TypeScript SDK
...
Database creation is asynchronous. Poll [`tenants.status`](/api-reference/v2/endpoint/tenant-status) until provisioning completes:
...
while True:
    status = client.tenants.status(database="my_first_tenant")
