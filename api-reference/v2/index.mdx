...
| [Sources](/api-reference/v2/endpoint/sources-overview) | Ingest, list, fetch, delete, and inspect knowledge or memories | Every time data flows into HydraDB  -  file uploads, app-uploads, user memories, and lifecycle ops |
| [Search](/api-reference/v2/endpoint/search-overview) | Retrieve context with hybrid, text, or vector search | At query time  -  the only endpoint you call to feed an LLM |
...
| [Knowledge](/essentials/v2/knowledge) | Shared source material such as PDFs, docs, app pages, tickets, Slack threads, or webpages. | Use `type=knowledge` when many users or agents should search the same content. |
...
| [`/search`](/api-reference/v2/endpoint/search) | `POST` | `search.query` | Unified search over knowledge, memories, or both | You need retrieval with `hybrid`, `text`, or `vector` search across `knowledge`, `memories`, or `all`. |
...
**Response envelope:** Raw HTTP responses are wrapped in a consistent envelope. Endpoint pages show the full envelope; the resource-specific payload lives under `data`.
...
- **Tenant scoping.** Most tenant-scoped endpoints require a `tenant_id`. Many source and search endpoints also accept an optional `sub_tenant_id` for finer-grained scoping. If omitted, the default sub-tenant is used.
...
- **Search modes.** `POST /search` supports `search_by: "hybrid"`, `"text"`, or `"vector"` and `source: "knowledge"`, `"memories"`, or `"all"`.
...
- **Understand the model:** [Core Concepts](/get-started/core-concepts) explains tenants, memories, search, and metadata
- **Go deeper:** [Essentials](/essentials/v2/search) covers each primitive in depth
