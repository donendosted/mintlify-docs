...
| [Query](/api-reference/v2/endpoint/query-overview) | Retrieve context with hybrid or text search | At query time  -  the only endpoint you call to feed an LLM |
...
| [`/query`](/api-reference/v2/endpoint/query) | `POST` | `query` | Unified search over knowledge, memories, or both | You need retrieval with `hybrid` or `text` search across `type: "knowledge"`, `type: "memory"`, or `type: "all"`. |
...
**Response envelope:** Core v2 endpoints (`/tenants`, `/source/*`, and `/search`) return a consistent envelope. Endpoint pages show the full envelope; the resource-specific payload lives under `data`. Webhook management endpoints (`/webhooks/indexing*`) are the exception: they return their documented response object directly.
...
- **Search modes.** `POST /query` supports `query_by: "hybrid"` or `"text"` and `type: "knowledge"`, `"memory"`, or `"all"`. The same `type` enum is used across ingestion, listing, deletion, and search; search additionally accepts `"all"`.
...
- **Understand the model:** [Core Concepts](/get-started/v2/core-concepts) explains tenants, memories, search, and metadata
