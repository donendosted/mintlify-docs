...
curl -X POST 'https://api.hydradb.com/connectors/{id}/resources' \
  ...
  -d '{
    ...
    "sub_tenant_id_override": "all-hands"  // deprecated — use `collection_override`
  }'
```
...
| <Field name="sub_tenant_id_override" type="string" /> | Routes synced objects from this resource into a specific sub-tenant partition. (deprecated — use `collection_override`) |
...
```json 201
{
  ...
  "status": "active",
  "provider_cursor": "",
  "tenant_id_override": "",          // deprecated — use `database_override`
  "sub_tenant_id_override": "all-hands",  // deprecated — use `collection_override`
...
<Note>
The request and response include `sub_tenant_id_override` and `tenant_id_override` for backward compatibility. The canonical names are `collection_override` and `database_override`.
</Note>
