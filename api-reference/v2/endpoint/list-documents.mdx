...
| --- | --- | --- |
| <Field name="metadata" type="object" /> | Context item's schema-aligned `metadata` payload | Keys must be declared in the database's `tenant_metadata_schema` with `enable_match: true`. Undeclared keys are silently ignored. |
