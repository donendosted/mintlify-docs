...
Use this endpoint to add new fields to a database's `tenant_metadata_schema`.

```http
PATCH /databases/{database}/metadata-schema
```
...
Each `add_fields[]` item uses the same field shape as `tenant_metadata_schema` on [Create Database](/api-reference/v2/endpoint/create-tenant):
