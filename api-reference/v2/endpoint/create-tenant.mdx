...
| <Field name="tenant_metadata_schema" type="array" /> | Defines database-level metadata fields. See the [Scoping using metadata](/essentials/v2/metadata#schema-declaration-set-at-tenant-creation) guide for detailed schema parameters. (default=`null`) |
...
<Warning>
  Schema field names are **immutable** after database creation. You can add per-document free-form metadata fields at ingestion time, but database-level fields cannot be mutated. Plan your schema carefully before creating the database.
</Warning>
