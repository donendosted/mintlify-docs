...
| <Field name="graph_payload" type="string (JSON map)" /> | Map of source id → your own entities + relations, **knowledge only** — replaces LLM graph extraction for each keyed source. Key = a `document_metadata` id or `app_knowledge` item id. See [Bring Your Own Graph](/essentials/v2/bring-your-own-graph) and the shape below. |
...
<Accordion title="Bring your own graph - supply entities and relations">
  ...
  | Field | Description |
  | --- | --- |
  | <Field name="<source_id>" type="object" /> | Top-level key: a `document_metadata` id or `app_knowledge` item id. Value is that source's graph. A key matching no source returns `400`. |
  ...
  <Note>
  **Per-source replace mode.** Each top-level key must match a `document_metadata` id or `app_knowledge` item id in the same request; attach graphs to multiple sources at once. Extraction is skipped for keyed sources. Caps per graph: ≤ 5,000 entities, ≤ 10,000 relations, ≤ 500 relations per entity; over-cap returns `400`. Graphs survive re-ingest (re-upload or connector re-sync re-applies the stored graph).
  </Note>
</Accordion>
