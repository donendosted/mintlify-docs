...
|---|---|---|---|---|
| `/search` | `POST` | `search.query` | Unified retrieval over knowledge, memories, or both | Ranked chunks + optional graph context |
...
| `sub_tenant_id` | default | Optional sub-tenant scope |
| `query` | - | Search terms or natural-language question |
| `source` | `sources` | Search `sources`, `memories`, or `all` |
| `search_by` | `hybrid` | Search using `hybrid`, `text`, or `vector` |
| `operator` | `or` | Text search operator for `search_by: text` |
