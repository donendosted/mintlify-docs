...
- **`mode`** picks **how** to rank results: `"fast"` (single-pass, low-latency, **the default if `mode` is omitted**), `"thinking"` (expands query, reranks, and can include forceful-relation context), or `"auto"` (scores the query and routes to `"fast"` or `"thinking"` automatically, defaulting to `"thinking"` when the signal is inconclusive).
...
<AccordionGroup>
  ...
  <Accordion title="Modes">
    ...
    |---|---|---|
    | `"fast"` *(default if `mode` is omitted)* | Single query pass | Real-time chat, autocomplete, simple lookups. |
    ...
    | `"auto"` | Scores the query before retrieval and routes to `"fast"` or `"thinking"`; defaults to `"thinking"` when the signal is inconclusive. Also overrides `graph_context` to match whichever mode it picks. | Mixed or unpredictable query traffic where you don't want to hand-pick per request. |

    `"auto"`'s resolved pipeline isn't reported back in the response, so budget latency as thinking-level in the worst case. `mode` is never inferred as `"auto"` on its own - if you omit `mode`, you get `"fast"`, not automatic routing.
  </Accordion>
...
- **`mode: "auto"` overrides `graph_context`.** Whatever you send for `graph_context` is replaced to match the resolved mode  -  `true` if auto escalates to `thinking`, `false` if it resolves to `fast`. Set `graph_context` explicitly only when calling `"fast"` or `"thinking"` directly.
- **Omitting `mode` is not the same as `mode: "auto"`.** No `mode` field at all defaults to `"fast"`. Automatic routing only happens if you send `mode: "auto"` explicitly.
