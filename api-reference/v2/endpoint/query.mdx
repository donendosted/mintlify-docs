...
| <Field name="collection" type="string or null" /> | Collection scope. Required for per-user memory queries. Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). (default=default collection) |
...
<AccordionGroup>
  ...
  <Accordion title="Filters">
    ...
    |---|---|
    | **Top-level keys** (`department`, `region`) | The source's `metadata`. Keys must be declared in the database's `tenant_metadata_schema` with `enable_match: true`, otherwise they are silently ignored. |
