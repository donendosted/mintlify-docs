...
- **Knowledge vs. Memories**: [Knowledge](/essentials/v2/knowledge) is shared, tenant-wide content (files, app pages, Slack messages). [Memories](/essentials/v2/memories) are user-specific preferences and conversational traits scoped by `sub_tenant_id`. Both can be searched together via `source: "all"`.
- **Source IDs**: Unique identifiers returned by `/source/ingest`. You can assign custom IDs using `file_id` in metadata or `id` in app knowledge. Use them for polling status, fetching content, and deleting sources.
