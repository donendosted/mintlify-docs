...
Pass one or more `source_id` values in `ids` to retrieve status. Works for documents, app sources, and memories. For more information, see the [Knowledge](/essentials/v2/knowledge) and [Memories](/essentials/v2/memories) guides.
...
| Name | Description |
| --- | --- |
| <Field name="ids" type="string[]" required /> | One or more `source_id` values returned at ingestion. Accepts IDs of any source  -  documents, app sources, memories. |
| <Field name="tenant_id" type="string" required /> | Tenant the items belong to. Provide at least ID. |
