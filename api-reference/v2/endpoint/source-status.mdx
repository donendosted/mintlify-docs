...
Required fields are marked with `*`. Provide `tenant_id` and at least one source ID using `file_ids` or the deprecated single `file_id`.

For multiple IDs over raw HTTP, repeat the query parameter:

```bash
/source/status?tenant_id=acme_corp&file_ids=policy_main&file_ids=runbook_deploy
```
