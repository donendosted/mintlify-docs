...
|---|---|---|
| `file_id` | string | **Deprecated** — single ID. Use `file_ids` instead. |
| `file_ids` | string[] | One or more `source_id` values returned at ingestion. Despite the name, accepts IDs of any source — files, app sources, memories. (default=`[]`) |
| `tenant_id` | string | Tenant the items belong to. (default=`""`) |
