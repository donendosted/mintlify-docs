...
HydraDB core v2 endpoints (`/tenants`, `/source/*`, and `/search`) use the same top-level envelope for successful and failed requests. Webhook management endpoints (`/webhooks/indexing*`) return their documented response object directly and do not include this envelope.
...
| `409` | Conflict, usually an existing tenant or source ID | Usually no |
...
try {
  await client.source.ingest({
    type: "knowledge",
    tenant_id: "my_first_tenant",
    files: [{ path: "policy.pdf", filename: "policy.pdf" }],
  });
} catch (error) {
...
try:
    ...
    type="knowledge",
        tenant_id="my_first_tenant",
        files=[("policy.pdf", open("policy.pdf", "rb"), "application/pdf")],
    )
except HydraDBError as exc:
...
Tenant creation is asynchronous. After `POST /tenants`, poll [`GET /tenants/status`](/api-reference/v2/endpoint/tenant-status) until `infra.scheduler_status`, `infra.graph_status`, `infra.vectorstore_status.knowledge`, and `infra.vectorstore_status.memory` are all `true`.

### Ingestion validation errors

Common causes:

- `file_metadata` length does not match the `files` array length.
- `app_knowledge`, `memories`, or `file_metadata` was sent as an object instead of a JSON-stringified multipart field.
...
- Source status may still be `queued` or `processing`; poll [`GET /source/status`](/api-reference/v2/endpoint/source-status).
...
- [Source Status](/api-reference/v2/endpoint/source-status) - async ingestion state
