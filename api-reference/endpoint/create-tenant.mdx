...
- **data_type**: Milvus data type (see supported types below)
- **enable_match**: Whether this field supports exact keyword filtering (`TEXT_MATCH`)
- **enable_dense_embedding**: Whether this field is indexed for semantic (vector) search
- **enable_sparse_embedding**: Whether this field is indexed for BM25 keyword search

## Supported Metadata Types

HydraDB supports the following Milvus data types for metadata fields:

### Common Types

| `data_type`  | Description        | Use Cases                                          |
| ------------ | ------------------ | -------------------------------------------------- |
| `VARCHAR`    | Text (default)     | Labels, identifiers, categories, searchable fields |
| `INT64`      | 64-bit integer     | Numeric metrics, versions, priorities              |
| `INT32`      | 32-bit integer     | Smaller numeric values                             |
| `FLOAT`      | 32-bit float       | Decimal values                                     |
| `DOUBLE`     | 64-bit float       | High-precision decimals                            |
| `BOOL`       | Boolean            | Binary flags, status indicators                    |
| `JSON`       | Structured JSON    | Hierarchical / nested data                         |
| `ARRAY`      | Multi-value array  | Tags, categories, multi-value attributes           |

## Search Capability Flags

Understanding the capability flags is crucial for optimal metadata design:

**1. `enable_dense_embedding` — Semantic (vector) search**

- **Purpose**: Index this field for AI-powered semantic similarity search
- **Use When**: You want users to find documents by asking natural-language questions about this field
...
- **Purpose**: Enable `TEXT_MATCH` filtering for fast exact-match lookups
- **Use When**: You want to narrow results with precise string criteria
...
- **`VARCHAR`**: For text, labels, and fields that need semantic or keyword search
- **`INT64` / `INT32`**: For numeric metrics, versions, and priorities
- **`BOOL`**: For binary flags and status indicators
- **`FLOAT` / `DOUBLE`**: For decimal values
- **`JSON`**: For structured, hierarchical data
- **`ARRAY`**: For multi-value fields like tags and categories

**3. Search Capability Strategy**

- **`enable_dense_embedding: true`**: Fields users will query semantically ("find docs about X")
- **`enable_sparse_embedding: true`**: Fields needing BM25 keyword search (pair with dense for hybrid)
- **`enable_match: true`**: Fields used for precise exact-match filtering
- **Non-VARCHAR types** (`INT64`, `BOOL`, etc.): Filterable via Milvus scalar filtering natively — no flags needed
...
- Scalar filter on `is_confidential` (`data_type: "BOOL"`)
