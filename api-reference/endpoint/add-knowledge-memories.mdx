...
2. **App Sources Uploads**: Upload structured knowledge from workplace apps using the `app_sources` multipart form field with an AppKnowledgeModel JSON array
...
The `app_sources` parameter accepts an array of `AppKnowledgeModel` objects. Each object represents a knowledge source from a workplace app with comprehensive metadata and content.

### `app_sources` Array Structure
...
<Info>
  **Supported Apps**: The `app_sources` format supports knowledge from various workplace apps including Gmail, Slack, Notion, Drive, Jira, Confluence, and more. Each app type is processed using specialized pipelines to extract and normalize content effectively.
</Info>

<Warning>
  **Required Fields**: The `tenant_id` and `sub_tenant_id` fields are required for all app sources. Ensure these are provided for each item in the `app_sources` array.
</Warning>
...
For file uploads, the identifier is `file_id` from `file_metadata`. For app sources uploads, the identifier is `id` from the AppKnowledgeModel. When that identifier already exists, `upsert` (boolean, default `true`) controls the behavior:
...
- For app sources uploads, ensure `tenant_id` and `sub_tenant_id` are correct
