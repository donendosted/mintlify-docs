...
| `metadata` | object | Tenant-level or app-specific metadata. Use this for category, channel, project, space, workspace, or other provider context. |
| `additional_metadata` | object | Document-level metadata. Stored per-source; not filterable at recall time. |
