...
3. **`POST /source/ingest`** (`client.source.ingest()`): Ingest documents, app records, or memories under `type="knowledge"` or `type="memory"`.
4. **`GET /source/status`** (`client.source.status()`): Poll status using `source_ids` until `indexing_status` is `completed` or `graph_creation`.
5. **`POST /query`** (`client.query()`): Retrieve context via `type: "knowledge"`, `"memory"`, or `"all"`.


> **Important:** Prefer the v2 surface in this guide. Older v1 routes like `/recall/full_recall`, `/ingestion/upload_knowledge`, `/list/data`, and `/tenants/create` may still appear in the OpenAPI file for compatibility, but new integrations should use the consolidated v2 endpoints: `/tenants`, `/source/*`, and `/query`.
...
Core raw HTTP responses (`/tenants`, `/source/*`, and `/search`) are wrapped:
...
Use the same `type` enum across source lifecycle APIs and search. Search additionally accepts `"all"` to query knowledge and memories together.
...
- Request/response fields remain snake_case in both SDKs, e.g. `tenant_id`, `sub_tenant_id`, `page_size`, `query_by`.
...
| `additional_metadata` / `document_metadata` | free-form fields; keep as objects |
...
| `search_apps` | boolean | app-aware retrieval lane for app sources; still searches the full selected knowledge scope |
...
| App search | `type: "knowledge"`, `query_by: "hybrid"`, `mode: "thinking"`, `search_apps: true`; still searches non-app knowledge too |
...
## 13. v1 to v2 migration cheatsheet
...
## 14. Cookbook patterns

The v2 cookbooks show production patterns. Click the links below to read the raw Markdown guides with complete setup code and schemas:
...
- [ ] Using v1 endpoints for new code instead of `/tenants`, `/source/*`, `/query`.
...
## 16. SDK Method Reference

### 16.1 TypeScript SDK (v2)

| Method | Endpoint | Purpose |
|---|---|---|
| `client.source.ingest()` | `POST /source/ingest` | Ingest files, app sources, or memories |
| `client.source.status()` | `GET /source/status` | Check processing status of ingested items |
| `client.source.fetch()` | `GET /source/fetch` | Retrieve original source content or get a presigned URL |
| `client.source.list()` | `POST /source/list` | Browse knowledge or memories |
| `client.source.delete()` | `DELETE /source` | Delete sources or memories |
| `client.source.relations()` | `GET /source/relations` | Inspect graph relationships |
...
|---|---|---|---|
| Source | `client.source.ingest()` | `POST /source/ingest` | Ingest files, app sources, or memories |
| Source | `client.source.status()` | `GET /source/status` | Check processing status of ingested items |
| Source | `client.source.fetch()` | `GET /source/fetch` | Retrieve original source content or get a presigned URL |
| Source | `client.source.list()` | `POST /source/list` | Browse knowledge or memories |
| Source | `client.source.delete()` | `DELETE /source` | Delete sources or memories |
| Source | `client.source.relations()` | `GET /source/relations` | Inspect graph relationships |
...
### 16.3 Minimal cURL example

Use `source` on raw `/query` requests because that is the OpenAPI field. The raw API response is wrapped; read payloads from `.data`.
...
SOURCE_ID=$(curl -s -X POST 'https://api.hydradb.com/source/ingest' \
  ...
  -F 'memories=[{"source_id":"alex_pref_001","text":"Alex prefers concise technical answers.","infer":true,"user_name":"Alex"}]' \
...
curl -s -X POST 'https://api.hydradb.com/query' \
  ...
  -H "API-Version: 2" \
  -H "Content-Type: application/json" \
  -d "{\"tenant_id\":\"${TENANT_ID}\",\"sub_tenant_id\":\"${SUB_TENANT_ID}\",\"query\":\"How should I answer Alex?\",\"source\":\"memory\",\"query_by\":\"hybrid\",\"mode\":\"thinking\"}" \
  | jq '.data.chunks'
```

### 16.4 Raw HTTP: ingest app-source knowledge
