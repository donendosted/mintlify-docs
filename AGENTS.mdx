...
1. **`POST /databases`** (`client.tenants.create()`): Provision an isolated database workspace.
...
- `meta` may also carry an optional `deprecation` string (with a `Deprecation: true` response header) when a request uses a legacy `/tenants` route or the legacy `tenant_id`/`sub_tenant_id` fields. It is a non-breaking nudge — the status code is unchanged. Prefer the `/databases` routes and `database`/`collection` fields to avoid it. Treat `meta` as an open object: ignore keys you do not recognize.
...
|---|---|
| Create database | `POST /databases` · `client.tenants.create()` |
| Check database readiness | `GET /databases/status` · `client.tenants.status()` |
| List databases | `GET /databases` · `client.tenants.list()` |
| List collections | `GET /databases/collections` · TS `client.tenants.subTenants()` / Python `client.tenants.sub_tenants()` |
| Database stats | `GET /databases/stats` · `client.tenants.stats()` |
...
| `client.tenants.list()` | `GET /databases` | List all databases registered for the organization |
| `client.tenants.delete()` | `DELETE /databases` | Permanently remove a database |
| `client.tenants.status()` | `GET /databases/status` | Check provisioning readiness of a database's infrastructure |
| `client.tenants.subTenants()` | `GET /databases/collections` | List active collections |
| `client.tenants.stats()` | `GET /databases/stats` | Get database usage stats |
...
| Databases | `client.tenants.create()` | `POST /databases` | Create a new isolated workspace |
| Databases | `client.tenants.list()` | `GET /databases` | List all databases registered for the organization |
| Databases | `client.tenants.delete()` | `DELETE /databases` | Permanently remove a database |
| Databases | `client.tenants.status()` | `GET /databases/status` | Check provisioning readiness of a database's infrastructure |
| Databases | `client.tenants.sub_tenants()` | `GET /databases/collections` | List active collections |
| Databases | `client.tenants.stats()` | `GET /databases/stats` | Get database usage stats |
