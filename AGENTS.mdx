...
1. **`POST /databases`** (`client.tenants.create()`): Provision an isolated tenant workspace.
2. **`GET /databases/status`** (`client.tenants.status()`): Poll until `infra.ready_for_ingestion` is true.
...
|---|---|
| Create tenant | `POST /databases` · `client.tenants.create()` |
| Check tenant readiness | `GET /databases/status` · `client.tenants.status()` |
| List tenants | `GET /databases` · `client.tenants.list()` |
| List sub-tenants | `GET /databases/collections` · TS `client.tenants.subTenants()` / Python `client.tenants.sub_tenants()` |
| Tenant stats | `GET /databases/stats` · `client.tenants.stats()` |
...
1. **Tenant creation**: after `POST /databases`, poll `GET /databases/status` until infrastructure is ready.
...
- Python methods and fields: snake_case, e.g. `client.tenants.sub_tenants()`, `database`, `collection`, `page_size`, `query_by`.
- TypeScript methods and fields: camelCase, e.g. `client.tenants.subTenants()`, `database`, `collection`, `pageSize`, `queryBy`.
...
# 1. Create tenant.
client.tenants.create(database=database)

# 2. Wait until the tenant can accept data.
while True:
    status = client.tenants.status(database=database)
...
const database = "my_first_tenant";

await client.tenants.create({ database: database });

while (true) {
  const status = await client.tenants.status({ database: database });
...
`POST /databases` · `client.tenants.create()`
...
- `POST /databases` may return `409 TENANT_ALREADY_EXISTS` for duplicate tenant IDs and `403 FORBIDDEN` when the API key or plan cannot create more tenants.

### Check readiness

`GET /databases/status?database=...` · `client.tenants.status()`
...
| Endpoint | Purpose |
|---|---|
| `GET /databases` | list tenant IDs |
| `DELETE /databases?database=...` | delete a tenant and all data |
| `GET /databases/collections?database=...` | list active sub-tenants |
...
| `client.tenants.create()` | `POST /databases` | Create a new isolated workspace |
| `client.tenants.list()` | `GET /databases` | List all tenants registered for the organization |
| `client.tenants.delete()` | `DELETE /databases` | Permanently remove a tenant |
| `client.tenants.status()` | `GET /databases/status` | Check provisioning readiness of a tenant's infrastructure |
| `client.tenants.subTenants()` | `GET /databases/collections` | List active sub-tenants |
| `client.tenants.stats()` | `GET /databases/stats` | Get tenant usage stats |
...
| Tenants | `client.tenants.create()` | `POST /databases` | Create a new isolated workspace |
| Tenants | `client.tenants.list()` | `GET /databases` | List all tenants registered for the organization |
| Tenants | `client.tenants.delete()` | `DELETE /databases` | Permanently remove a tenant |
| Tenants | `client.tenants.status()` | `GET /databases/status` | Check provisioning readiness of a tenant's infrastructure |
| Tenants | `client.tenants.sub_tenants()` | `GET /databases/collections` | List active sub-tenants |
| Tenants | `client.tenants.stats()` | `GET /databases/stats` | Get tenant usage stats |
