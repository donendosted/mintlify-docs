...
- Run natural language workplace search across all sources with a single [`/search`](/api-reference/v2/endpoint/search) call
...
> **Best Practice**: Always verify processing after upload using the `/source/status` endpoint to ensure your data is properly indexed.
...
def search_with_memory(query: str, tenant_id: str, user_id: str):
    ...
    search_results = client.search.query(**kwargs)
...
class DataPrivacyManager {
  ...
  async deleteOldData(tenantId, memoryId, subTenantId = null) {
    ...
    let url = `https://api.hydradb.com/source?tenant_id=${tenantId}&memory_id=${memoryId}`;
    if (subTenantId) {
...
- **Processing Verification**: Always verify upload processing using `/source/status`
...
| Date | Change |
|------|--------|
| 2026-05-14 | Replaced raw `fetch()`-based `HydraDBDataIngestion` class with official SDK calls (`client.source.ingest`, `client.source.status`) |
| 2026-05-14 | Replaced raw `fetch()`-based `GleanSearch` class with `client.search.query` SDK call |
...
| 2026-05-14 | Replaced webhook `cortexIngestion.uploadBatch()` call with `client.source.ingest()` SDK call |
