...
> **Goal**: Ingest six source types into one HydraDB tenant, store per-user memory profiles for personalized answers, and answer three query patterns - simple factual lookup, decision provenance, and cross-source synthesis - all through `POST /search`.
...
graph LR
    ...
    B -->|"POST /source/ingest"| C["HydraDB<br/>tenant: company-knowledge<br/>sub_tenants: slack, email, docs, github"]
    D["User / Slack bot / Web UI"] -->|"POST /search"| C
    C -->|"ranked chunks + graph_context"| D
    E["POST /source/ingest"] -->|"user profile"| C
    C -->|"POST /search"| D
```

- **Ingestion Layer**: Six connector scripts that format source content and upload to HydraDB via `POST /source/ingest` using multipart form-data.
...
- **User Memory**: Per-user profiles stored via `POST /source/ingest` and retrieved via `POST /search` to personalize answer depth and format.
...
await client.tenants.create({ tenant_id: TENANT_ID });
```
...
All connectors use the same endpoint: `POST /source/ingest`. This endpoint uses **multipart form-data** - not JSON. `tenant_id` and `sub_tenant_id` are form fields alongside the file.
...
> **Batch limit**: Max 20 sources per request. Wait 1 second between batches. Always call `GET /source/status` before querying newly ingested content.
...
def _upload_batch(batch: list, sub_tenant: str) -> list:
    ...
    for filename, content in batch:
        ...
        if items:
            source_ids.append(items[0].source_id)
...
def ingest_gmail_threads(credentials_path: str, query: str, max_threads: int = 200):
    ...
    for thread in threads:
        ...
        items = result.results or []
        if items:
            all_ids.append(items[0].source_id)
...
After uploading, poll `GET /source/status` until `indexing_status` is `completed` before running any queries. HydraDB indexes asynchronously - typically 10–30 seconds per file.

> **Note**: [`GET /source/status`](/api-reference/v2/endpoint/source-status) takes `file_ids` and `tenant_id` as **query parameters**. Pass multiple `file_ids` to check a batch in one call.

```bash
curl -X POST \
  'https://api.hydradb.com/source/status?tenant_id=company-knowledge&file_ids=YOUR_SOURCE_ID' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
async function waitUntilIndexed(
  ...
  for (let i = 0; i < maxTries; i++) {
    ...
    const result = await client.source.status({
      tenant_id: TENANT_ID,
...
Three query patterns cover every internal search use case. All use `POST /search` with different parameters. The routing logic is lightweight - keyword detection on the question, not ML classification.

> **Important**: Use `POST /search` with a `query` field for all three query patterns below. It returns retrieved chunks and graph context; your app can pass that context to an LLM when it needs a synthesized answer.
...
async function search(
  ...
  return await client.search.query(payload);
}
...
def get_user_profile(user_id: str) -> str:
    """Retrieve a user's stored memory profile via /search with source: "memories"."""
...
def explain_decision(question: str, user_id: str) -> str:
    """
    Answer 'why' / 'what led to' questions with full provenance.
    Step 1: search chunks + graph relations from /search (source="knowledge").
    Step 2: retrieve user profile from /search (source="memories").
    Step 3: synthesize with citations via LLM.
    """
    # Step 1: Search with graph context
    ...
    chunk_relations = (data.graph_context.chunk_relations if data.graph_context else [])
...
def smart_search(question: str, user_id: str) -> dict:
    ...
    return {"chunks": result.chunks or [], "type": "search"}
```
...
To personalize any answer, retrieve the user's stored memory profile before calling the LLM. This is the same response structure as any [`/search`](/api-reference/v2/endpoint/search) call  -  an array of `chunks`.
...
> **Reading the response**: [`/search`](/api-reference/v2/endpoint/search) returns the same structure regardless of `source`  -  read the profile from `chunks[0].chunk_content`. `additional_metadata` will be `null` for memory entries; that's expected.
...
| `POST` | `/source/ingest` | Upload a source file (multipart form-data) |
| `POST` | `/source/status?tenant_id=...&file_ids=...` | Check indexing status |
| `POST` | `/source/ingest` | Store a user profile memory |
| `POST` | `/search` | Retrieve user profile for personalization |
| `POST` | `/search` | Query all indexed knowledge |
...
```
GET /source/status?tenant_id=company-knowledge&file_ids=YOUR_SOURCE_ID
Body: {}
```
...
├── ingest/
│   └── verify.py                 # poll /source/status until indexed
├── memory/
