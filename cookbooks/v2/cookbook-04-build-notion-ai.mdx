...
def upload_batch(sources: list, sub_tenant_id: str = None) -> list:
    """Upload up to 20 sources. Returns list of file IDs."""
    if sub_tenant_id:
        for s in sources:
...
Confluence pages follow the same upload format. Use `type: "confluence"` so HydraDB can distinguish sources during recall and apply metadata filtering.
...
**Endpoint:** `GET /source/status?file_ids=FILE_ID&tenant_id=TENANT` - Check indexing status

```python
def poll_until_indexed(file_id: str, timeout: int = 120, interval: int = 3):
...
**Endpoint:** `POST /source/ingest` - Add a user memory
...
def add_user_memory(
    ...
    return client.source.ingest(**payload)
...
This is the call your agent makes before answering any question. `POST /search` runs HydraDB's full multi-stage pipeline and returns ranked, contextually relevant chunks - including graph context showing relationships between entities.

**Endpoint:** `POST /search` - Retrieve agent context
...
def recall_context(
    ...
    To also retrieve user memories, call /search separately.
    ...
    return client.search.query(**payload)
    # Response shape:
...
For conversational, AI-generated answers, first retrieve context with `POST /search`. Then pass the returned `chunks` and `sources` into your application-layer LLM prompt. Key parameters: `alpha` (0-1, balance semantic vs lexical), `recency_bias` (0-1, prefer newer content), and `graph_context`.

**Endpoint:** `POST /search` - retrieved context for app-layer answer generation
...
**POST** `/source/ingest` - Max 20/call, 1s between batches
...
**POST** `/source/ingest` - Single file with tenant_id as form field
...
**POST** `/source/status?file_ids=FILE_ID&tenant_id=YOUR_TENANT` - Poll until status = "completed"

### Full recall

**POST** `/search` - Searches knowledge base - returns chunks + graph_context
...
**POST** `/search` - retrieved context for app-layer answer generation
...
**POST** `/source/ingest` - memories[] array wrapper - consistent SDK format
...
**POST** `/search` - user_name key - consistent across all calls
...
**DELETE** `/source` - Remove stale or incorrect data by memory_id
