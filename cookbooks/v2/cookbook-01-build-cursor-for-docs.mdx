...
```
Accepted: {'database': 'engineering-docs', 'status': 'accepted', 'message': 'Tenant accepted. Poll /databases/status for readiness.'}
...
**If it fails:**
- `404 Tenant does not exist` - Step 1 not complete, or `database` mismatch.
...
**Fix:** Run `python3 phase0/create_tenant.py` and wait for "✓ Tenant ready." before proceeding. Confirm `DATABASE_ID` in `config.py` exactly matches what you passed to `create_tenant`.
...
Response:
{
  "database": "engineering-docs",
  "status":    "accepted",
  "message":   "Tenant accepted. Poll /databases/status for readiness."
}
