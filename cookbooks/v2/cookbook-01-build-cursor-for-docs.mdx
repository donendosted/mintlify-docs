...
**What:** Uploads a single file using **multipart form-data** to `/source/ingest` - the recommended beginner path. HydraDB handles chunking, embedding, and graph-node creation automatically. The returned `source_id` is what you use in Step 3 to verify indexing.

> 💡 **Two ingestion modes exist.** This step uses **multipart file upload via `/source/ingest`** - the tested beginner path. An advanced JSON body mode (used in Phases 1–2) supports structured IDs and explicit graph `relations`. Use file upload here first.
...
**What:** Polls `/source/status` using the `source_id` returned in Step 2. HydraDB returns a `statuses` array; you read `statuses[0].indexing_status`. Querying before this reaches `"completed"` returns empty results with no error - the most common beginner confusion.
...
def verify_file(file_id: str, timeout: int = 120, interval: int = 3):
    """
    Polls /source/status until indexing_status is "completed" or "errored".
    """
...
**What:** Sends a POST to `/search` and receives a `chunks` array. Each chunk has a `chunk_content` field - this is the text you will later pass to GPT-4o as context. This is the end-to-end proof that tenant, ingestion, indexing, and retrieval all work.
...
> ✅ **Phase 0 complete.** The same `/search` endpoint - with the same minimal three-field body - is what every later phase uses. You'll only add parameters, not change the structure. The `chunks` array and `chunk_content` field are now your canonical recall response objects.
...
**Chunks vs. sources:** When HydraDB indexes a document, it splits the content into overlapping **chunks**. Each chunk is embedded independently and stored as a node in the context graph. When you call `/search`, you get back a `chunks` array. Iterate this array and read `chunk_content` from each item - that is the text you pass directly to your LLM as context.
...
**What:** Switches to **JSON body ingestion** via `/source/ingest` with `app_knowledge` field - the advanced ingestion path. This gives you full control over IDs, timestamps, collections, metadata, and (in Phase 2) explicit graph `relations`. The `{repo_name}/{relative_path}` ID convention is required for Phase 2 graph linking.
...
**What:** `collections` are labels you define for scoping recall. `additional_metadata` fields are arbitrary key-value pairs used for filtering and citation labels. No extra API calls needed - these fields go in the ingestion payload.
...
- **`hydra_client.py`** - `search_docs()` is the HydraDB retrieval path. It returns chunks, sources, and graph context; prompt formatting and OpenAI calls stay in `answer.py`. Imported by → `recall.py`, `app.py`.
- **`recall.py`** - `recall_context()` calls `search_docs()` and returns the raw payload. `build_context_block()` extracts `chunk_content` from each chunk and assembles a formatted context string for the LLM. Imported by → `app.py`.
...
def search_docs(
    ...
    Calls /search. scope (sub_tenant_id) is optional - omit to
...
1. **Read the `chunks` array** from the recall payload. `/search` returns `{"chunks": [...], "sources": [...]}` - always iterate `chunks`, never `results`.
...
**Fix:** Correct paths: `/source/ingest` (file upload or JSON batch with `app_knowledge`), `/search`, `/source/status`. If using `sub_tenant_id`, confirm at least one batch was successfully uploaded to that sub-tenant first.
...
- **Indexing delays.** Indexing is async. Never rely on a fixed sleep; always poll [`/source/status`](/api-reference/v2/endpoint/source-status) until the `statuses` array shows `"indexing_status": "completed"`.
...
**POST** `/source/ingest` - Multipart form-data · field name `"files"` · tenant_id as form field
...
**POST** `/source/ingest` - Max 20 items · body field `app_knowledge` (JSON string) · supports relations
...
**POST** `/source/status` - Returns `statuses[]` array · poll until `"completed"`
...
**POST** `/search` - Primary verified path · `sub_tenant_id` optional · returns `chunks[]` and `sources[]`
...
**POST** `/search` - Use for multi-hop retrieval, then generate the answer in your app layer
