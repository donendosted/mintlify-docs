...
Earnings PDFs are the primary source. Tag each with structured metadata - `ticker`, `period`, `doc_type`, `fiscal_year`, `fiscal_quarter` - so you can filter search to a specific company or time range before semantic search even runs.
...
def query_trend(
    ...
    chunks      = search.chunks or []
    graph_ctx   = search.graph_context
