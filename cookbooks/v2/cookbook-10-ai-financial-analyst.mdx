...
- **`graph_context: true`** on `/search` gives you `query_paths` - the entity relationship chains that show how a metric's value evolved across documents.
...
def ingest_earnings_pdf(
    ...
    file_id = result.results[0].source_id if result.results else str(result)
...
def ingest_metrics_series(metrics_by_period: list[dict]) -> list[str]:
    ...
    Uses /source/ingest (not upload_knowledge) because metrics are
    ...
    for item in metrics_by_period:
        ...
        memory_ids.append(result.results[0].source_id if result.results else "ok")
...
def verify_all_indexed(file_ids: list[str], poll_interval: int = 3, max_polls: int = 20) -> bool:
    """
    Poll /source/status until all file_ids report status 'completed'.
...
> **Batch limit reminder.** Maximum 20 sources per request. Wait 1 second between batches. Call [`/source/status`](/api-reference/v2/endpoint/source-status) before any production query.
...
def query_trend(
    ...
    chunks      = recall.chunks or []
    graph_ctx   = recall.graph_context
    ...
    analyst_prefs = client.search.query(
        tenant_id=TENANT_ID,
        sub_tenant_id=f"analyst-{user_id}",
        mode="thinking",
...
For analyst chat interfaces, use `POST /search` to retrieve chunks, sources, and graph context. Generate the final answer in your application layer with your LLM provider so citations, formatting, and conversation memory stay under your control.
...
def financial_recall(
    ...
    result = client.search.query(**payload)
...
print(f"Chunks: {len(r2.chunks or [])}")
...
def generate_quarterly_briefing(
    ...
    current_chunks = current_q.chunks or []
    ...
    store_resp = client.source.ingest(
        tenant_id=TENANT_ID,
        sub_tenant_id=f"analyst-{analyst_user_id}",
        upsert=True,
        memories=[{
...
curl -X POST 'https://api.hydradb.com/source/ingest' \
  ...
  -F "files=@ACME_Q2_2023_transcript.pdf" \
...
```http
GET /source/status?file_ids=FILE_ID&tenant_id=financial-analyst
```
...
| Raw `requests` for PDF upload | 422 error on `/source/ingest` | Use the SDK: `pip install hydradb-sdk` |
| No `/source/status` polling | Queries return empty results silently | Always poll status before querying |
