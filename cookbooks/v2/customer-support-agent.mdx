...
- **Two-call search pattern** - when a customer opens a ticket, the agent calls `POST /search` to search the knowledge base and `POST /search` to retrieve personal memory. Results are merged before the LLM call. Use `mode: "thinking"` on both calls to enable personalised ranking.
...
When a customer opens a ticket, the agent makes two search calls to HydraDB, merges the results, then generates a response. `/search` searches the knowledge base; `/search` searches the customer's personal memory. Both are needed - neither alone returns the full picture.
...
def recall_customer_context(
    ...
    """
    Two-call search pattern:
    1. search (source="knowledge") - searches knowledge base (docs, resolved tickets)
