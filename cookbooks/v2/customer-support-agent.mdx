...
- Make the two-call recall pattern - knowledge base + customer memory - on every incoming ticket
...
1. `POST /search` - retrieves knowledge base context: help articles, past ticket resolutions, and linked documents relevant to the customer's message.
2. `POST /search` - retrieves the customer's personal memory: their plan, past issues, inferred preferences, and conversation history.
...
- **Knowledge memories** - help docs, FAQs, past ticket resolutions. Uploaded once via `client.source.ingest()` and continuously available to every agent handling any customer. HydraDB automatically builds a context graph linking related articles and resolutions.
- **User memories** - per-customer context stored via `POST /source/ingest` with the customer's `user_name`. Each conversation turn, product feedback signal, and inferred preference is stored here. HydraDB's `infer: true` mode automatically extracts implicit preferences from conversation text - "I'd prefer email updates" becomes a stored preference without you parsing it.
- **Two-call recall pattern** - when a customer opens a ticket, the agent calls `POST /search` to search the knowledge base and `POST /search` to retrieve personal memory. Results are merged before the LLM call. Use `mode: "thinking"` on both calls to enable personalised ranking.
...
graph LR
    ...
    B -->|"POST /search"| C["HydraDB\nKnowledge Base"]
    B -->|"POST /search"| D["HydraDB\nCustomer Memory"]
...
async function uploadKbBatch(batch: object[]): Promise<string[]> {
    ...
    const result = await client.source.ingest({
        ...
        upsert:        true,
        app_knowledge: JSON.stringify(batch),   // JSON string, not an array
    });
    return (result.results || []).map((r: any) => r.source_id);
}
...
async function storeConversationTurn(
    ...
    await client.source.ingest({
        tenant_id:     TENANT_ID,
        sub_tenant_id: `customer-${customerId}`,
        memories: [{
            text,
...
When a customer opens a ticket, the agent makes two recall calls to HydraDB, merges the results, then generates a response. `/search` searches the knowledge base; `/search` searches the customer's personal memory. Both are needed - neither alone returns the full picture.
...
async function recallCustomerContext(
...
): Promise<{ chunks: any[]; graph_context: any }> {
    /**
    ...
    const kbData = await client.search.query({
        tenant_id:     TENANT_ID,
        sub_tenant_id: KB_SUB_TENANT,
        query:         customerMsg,
    ...
    const memData = await client.search.query({
        tenant_id:     TENANT_ID,
        sub_tenant_id: `customer-${customerId}`,
        query:         customerMsg,
        max_results:   8,
    ...
    return {
        chunks:        (memData.chunks || []).concat(kbData.chunks || []),
        graph_context: kbData.graph_context || {},
    };
...
def recall_customer_context(
    ...
    """
    Two-call recall pattern:
    1. search (source="sources") - searches knowledge base (docs, resolved tickets)
    2. search (source="memories") - searches customer's personal memory
    Merge both before passing to LLM.
    mode: "thinking" enables personalised ranking on both calls.
    sub_tenant_id scopes each call to the right data store.
    """
    # Call 1: knowledge base - help docs, resolved tickets, FAQs
    kb_data = client.search.query(
        tenant_id=TENANT_ID,
        sub_tenant_id=KB_SUB_TENANT,
        query=customer_msg,
    ...
    # then knowledge base chunks, then combined graph context
    return {
        "chunks":        (mem_data.chunks or []) + (kb_data.chunks or []),
        "graph_context": kb_data.graph_context or {},
    }
    # chunks[n]["chunk_content"]   - the actual text
    # chunks[n]["source_title"]    - which doc or memory it came from
    # chunks[n]["relevancy_score"] - HydraDB's confidence (0–1)
...
async function handleTicket(
    ...
    const graphCtx    = contextData.graph_context || {};
    ...
    const entityPaths = (graphCtx.query_paths || []).slice(0, 3);
...
> **Alternative - skip the LLM**: Use `POST /search` with `mode: "thinking"` and `sub_tenant_id: customer_id` to have HydraDB generate the answer directly. Faster, but less control over the system prompt and tone.
...
async function escalateToHuman(
    ...
    const customerProfile = await client.search.query({
        tenant_id:     TENANT_ID,
        sub_tenant_id: `customer-${customerId}`,
    ...
    const similarData = await client.search.query({
        tenant_id:   TENANT_ID,
        query:       `${customerMsg} resolved escalated human agent`,
        max_results: 5,
    });
    const similarTickets = similarData.chunks || [];
...
def escalate_to_human(
    ...
    """
    # Recall full customer memory profile
    customer_profile = client.search.query(
        tenant_id=TENANT_ID,
        sub_tenant_id=customer_sub_tenant(customer_id),
        query="customer account history preferences past issues plan",
    )
    ...
    # Recall similar past tickets resolved by humans
    similar_data = client.search.query(
        tenant_id=TENANT_ID,
        query=f"{customer_msg} resolved escalated human agent",
        max_results=5,
    )
    similar_tickets = similar_data.chunks or []
...
| `POST` | `/source/ingest` | Upload help docs and past tickets (SDK only) |
| `POST` | `/source/status` | Check indexing status |
| `POST` | `/source/ingest` | Store conversation turns and preferences |
| `POST` | `/search` | Search knowledge base |
| `POST` | `/search` | Retrieve customer personal memory |
