...
```mermaid
graph LR
    A["Decision logs<br/>Org charts<br/>Meeting notes<br/>Product specs"] -->|"upload.knowledge()"| B["HydraDB<br/>tenant: onboarding<br/>sub_tenant: company-context"]
    C["New hire question"] -->|"search.query()"| B
    B -->|"retrieved context"| D["Onboarding agent"]
    D -->|"upload.add_memory()"| E["HydraDB<br/>sub_tenant: hire-{id}"]
    E -->|"search.query() with source: memories"| F["Manager dashboard"]
```

- **Phase 0**: Install SDK, create tenant, upload one document, run the first recall query.
...
- Verify indexing before running recall queries so new hires always get answers from complete data
...
def upload_doc(filepath: str, doc_id: str, doc_type: str, team: str):
    ...
    file_metadata must be a JSON string, not a dict.
    """
    with open(filepath, "rb") as f:
        result = client.source.ingest(
            tenant_id=TENANT_ID,
            files=[(os.path.basename(filepath), f, "application/octet-stream")],
...
> Phase 0 complete. The same `client.search.query()` retrieval pattern is used in every later phase. Your application can pass returned chunks into an LLM to generate the final answer.
...
> **Batch limit**: Max 20 files per `upload.knowledge()` call. For large document sets, upload in batches with a 1-second sleep between them.
...
def ingest_people(people: list):
    ...
    for i, p in enumerate(people):
        ...
        metadata_list.append({
            "file_id": f"person-{p['name'].lower().replace(' ', '-')}",
    ...
    try:
        result = client.source.ingest(
            tenant_id=TENANT_ID,
            files=file_handles,
            file_metadata=json.dumps(metadata_list),
        )
...
### Step 5 - Test Multi-source Recall
...
for question in TEST_QUESTIONS:
    ...
    top_chunk = result.chunks[0].chunk_content[:300] if result.chunks else "No context returned yet"
...
def ask_onboarding(hire_id: str, question: str, topic: str = "") -> str:
    ...
    company_context = client.search.query(
        tenant_id=TENANT_ID,
        query=question,
        max_results=5,
        mode="thinking",
    ...
    hire_context = client.search.query(
        tenant_id=TENANT_ID,
        sub_tenant_id=hire_sub(hire_id),
        query=question,
        max_results=3,
    ...
    return {
        "company_context": company_context.chunks,
        "hire_context": hire_context.chunks,
    }
...
| `422 Unprocessable Entity` on upload | Sending JSON text instead of files | Use `client.source.ingest(files=[...])` - not raw JSON body. |
| `422 field required: memories` on add_memory | Wrong field structure | Pass `memories=[{"text": "...", "infer": True}]` as a list of dicts. |
...
| Topic | Note |
|---|---|
| Batch size | Max 20 files per `upload.knowledge()` call. Sleep 1s between batches for large document sets. |
...
| `client.source.ingest(tenant_id=..., files=..., file_metadata=...)` | Upload company knowledge documents |
| `client.source.ingest(tenant_id=..., sub_tenant_id=..., memories=[...])` | Store per-hire memory |
| `client.search.query(tenant_id=..., query=..., max_results=..., mode=...)` | Retrieve company context for a hire question |
| `client.search.query(tenant_id=..., sub_tenant_id=..., query=...)` | Recall hire memory for manager dashboard |

### client.source.ingest - key parameters

| Parameter | Type | Notes |
|---|---|---|
| `tenant_id` | `str` | Required. Your tenant ID. |
| `files` | `list[IO[bytes]]` | Required. Open file handles - not file paths, not dicts. |
| `file_metadata` | `str` | Optional. JSON **string** (not dict) with per-file `file_id` and `additional_metadata`. |

### client.search.query - key parameters
...
| Cross-source recall (doc + meeting + org chart) | 12% | 78% | +550% |
