...
> **Goal**: Build an agent that ingests four signal types from competitor sources, verifies indexing, and answers competitive queries using `POST /search` with `recency_bias` tuned per query type - point-in-time or trend. Full round-trip under 200ms.
...
graph LR
    ...
    B -->|"POST /source/ingest<br/>multipart form-data"| C["HydraDB<br/>tenant: competitive-intel<br/>sub_tenant: acme-corp"]
    D["Analyst / Agent"] -->|"POST /search<br/>query + recency_bias"| C
...
- **Ingestion Layer**: Connector scripts that format content and upload to HydraDB via `POST /source/ingest`.
- **HydraDB**: Stores all signals with timestamps, builds a context graph, and ranks results by recency at query time.
- **Analyst / Agent**: A human analyst, a Slack bot, or an LLM that calls `POST /search` with a natural language query.
...
All four signal types use `POST /source/ingest`. This endpoint uses **multipart form-data**, not JSON. `tenant_id` and `sub_tenant_id` are form fields, not body keys.
...
> **Batch limit**: Max 20 sources per request. Wait 1 second between batches. Always call `GET /source/status` before running search - queries against unindexed sources return empty results.
...
After uploading, poll `GET /source/status` until `indexing_status` is `completed`. HydraDB indexes asynchronously - typically 10–30 seconds per file. Do not query until indexing is complete; unindexed sources return empty results.
...
```bash
curl -X POST \
  'https://api.hydradb.com/source/status?tenant_id=competitive-intel&file_ids=YOUR_SOURCE_ID' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
def wait_until_indexed(source_id: str, max_tries: int = 20, interval: int = 3) -> None:
    """
    Poll /source/status until the source is indexed or times out.
    Raises RuntimeError if indexing errors. Warns on timeout (may still complete).
    """
    for i in range(max_tries):
        time.sleep(interval)
        status_result = client.source.status(
            tenant_id=TENANT_ID,
            file_ids=[source_id],
        )
...
Use `POST /search` to answer "What is Competitor X doing right now?". Set `recency_bias: 0.8` so HydraDB strongly weights the most recent signals. Set `mode: "thinking"` to enable multi-query reranking.
...
def print_results(result) -> None:
    """Pretty-print chunks and graph entities from a /search response."""
...
Push a proactive Monday-morning briefing to each analyst's Slack channel. Each analyst profile runs a set of questions through [`/search`](/api-reference/v2/endpoint/search) with `recency_bias: 0.85` to surface the freshest signals.
...
def recall_for_question(question: str, competitor: str, recency_bias: float) -> str:
    """Run a single /search query and return the top chunks as a text block."""
...
| `POST` | `/source/ingest` | Upload a signal file (multipart form-data) |
| `POST` | `/source/status?tenant_id=...&file_ids=...` | Check indexing status |
| `POST` | `/search` | Query indexed signals |
...
```
GET /source/status?tenant_id=competitive-intel&file_ids=YOUR_SOURCE_ID
Body: {}
```
