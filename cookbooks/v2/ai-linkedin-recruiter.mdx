...
graph TD
    ...
    C["HydraDB APIs<br/>• Full Recall<br/>• AI Memories<br/>• Metadata Search"]
...
class AIRecruitingSearch {
  ...
  async findCandidates(query, recruiterContext = {}) {
    ...
    const results = await this.client.search.query({
      ...
      max_results: 20, // More candidates for review
  ...
  enhanceCandidateResults(results, originalQuery) {
    if (!results.chunks) return results;

    // Add hiring-specific analysis to each candidate
    const enhancedSources = results.chunks.map(candidate => ({
      ...candidate,
  ...
  async addPersonalizedInsights(results, recruiterId) {
    ...
    const personalizedResults = {
      ...
      personalized_insights: {
        recommended_candidates: this.getRecommendedCandidates(results.chunks, recruiterProfile),
        similar_to_past_hires: this.findSimilarToPastHires(results.chunks, recruiterProfile),
        interview_suggestions: this.generatePersonalizedInterviewQuestions(results.chunks, recruiterProfile),
...
const optimizationTips = {
  ...
  upload_strategy: {
    ...
    // verify: always call GET /source/status after upload
  },
