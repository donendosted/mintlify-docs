...
> **Cookbook 04** · Intermediate · 38 min read · Knowledge Management
...
def upload_batch(sources: list, sub_tenant_id: str = None) -> list:
    ...
    resp = requests.post(
        ...
        headers=HEADERS,
        json={"app_knowledge": sources},  # FIX: single key, correct wrapper
    )
    resp.raise_for_status()
    return resp.json().get("ids", [])  # FIX: "ids" not "results"
...
for chunk in context["chunks"]:
    print(f"[{chunk['relevancy_score']:.2f}] {chunk['source_title']}")
    print(chunk["chunk_content"][:200])
```
...
def handle_mention(event, client):
    ...
    answer = "\n\n".join(c.get("chunk_content", "") for c in chunks[:3]) or "No results found."
    sources = result.get("sources", [])

    if sources:
        links  = "\n".join(f"• {s.get('title','')}" for s in sources[:3])
        answer += f"\n\n*Sources:*\n{links}"
