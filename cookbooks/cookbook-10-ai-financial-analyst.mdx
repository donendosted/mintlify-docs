...
## Step 5 - Analyst Recall Interface

For analyst chat interfaces, use `POST /recall/full_recall` to retrieve chunks, sources, and graph context. Generate the final answer in your application layer with your LLM provider so citations, formatting, and conversation memory stay under your control.
...
def financial_recall(
    ...
    resp = requests.post(
        f"{BASE_URL}/recall/full_recall", headers=HEADERS, json=payload
    )
    resp.raise_for_status()
    result = resp.json()
...
def context_for_llm(result: dict) -> str:
    return "\n\n".join(
...
print(f"Chunks: {len(r2.get('chunks', []))}")
