...
> **Difficulty**: Intermediate · **Read time**: ~45 min · **Tags**: Finance, Temporal, Knowledge Graph
...
> **Note**: All API calls in this guide are real and ready to run. Base URL: `https://api.hydradb.com`. Get your API key by emailing [hello@hydradb.com](mailto:hello@hydradb.com) or booking a demo at [hydradb.com](https://hydradb.com).
...
graph TD
    ...
    F["Analyst Profile\n(/memories/add_memory)"] --> E
    E -->|"/recall/full_recall\nrecency_bias + graph_context"| G["Context Assembly"]
    E -->|"/search/qna\nmode: thinking"| H["Direct Answer"]
    G --> I["LLM Synthesis\n(gpt-4o / gemini)"]
...
- **`graph_context: true`** on `/recall/full_recall` gives you `query_paths` - the entity relationship chains that show how a metric's value evolved across documents.
...
# Output: 201  {'status': 'created', 'tenant_id': 'financial-analyst'}
...
> **SDK required for PDF ingestion.** The `/ingestion/upload_knowledge` endpoint requires `multipart/form-data`. Raw `requests` with `json=` returns a 422. Install: `pip install hydra-db-python`. Import as `from hydra_db import HydraDB` (the import name differs from the package name). For all recall and memory endpoints, raw `requests` continues to work.
...
Earnings PDFs are the primary source. Tag each with structured metadata - `ticker`, `period`, `doc_type`, `fiscal_year`, `fiscal_quarter` - so you can filter recall to a specific company or time range before semantic search even runs.
...
def ingest_earnings_pdf(
    ...
    """
    Upload a single earnings PDF. Returns the file_id for verification.
    ...
    with open(file_path, "rb") as f:
        # FIX: SDK parameter is 'files' (a list), NOT 'file' (a single handle).
        # Passing file=f raises a TypeError; passing files=[f] is correct.
        result = client.upload.knowledge(
            tenant_id=TENANT_ID,
            sub_tenant_id=sub_tenant_id,
            app_knowledge=app_knowledge,
            files=[f],          # <-- was: file=f  (wrong - causes TypeError)
        )

    file_id = getattr(result, "file_id", None) or getattr(result, "id", None) or str(result)
    print(f"Uploaded {ticker} {period_label} {doc_type} → file_id: {file_id}")
    return file_id
...
file_ids = ingest_earnings_batch(filings)
# Output:
# Uploaded ACME Q1 2023 earnings_transcript → file_id: 988293fa-e29
# Uploaded ACME Q2 2023 earnings_transcript → file_id: 9d7951c8-160
# Uploaded ACME Q3 2023 earnings_transcript → file_id: 2cf6ea6e-5fe
# Uploaded ACME Q4 2023 earnings_transcript → file_id: edff67d5-237
```
...
TENANT_ID = "financial-analyst"
HEADERS   = {
    "Authorization": f"Bearer {os.environ['HYDRADB_API_KEY']}",
    "Content-Type": "application/json",
}
...
def ingest_metrics_series(metrics_by_period: list[dict]) -> list[str]:
    ...
    Uses /memories/add_memory (not upload_knowledge) because metrics are
    ...
    for item in metrics_by_period:
        ...
        result = resp.json()
        memory_ids.append(result.get("id", "ok"))
...
def ingest_board_memo(
    ...
    with open(file_path, "rb") as f:
        # FIX: SDK parameter is 'files' (list), not 'file'.
        result = client.upload.knowledge(
            tenant_id=TENANT_ID,
            sub_tenant_id=sub_tenant_id,
            app_knowledge=app_knowledge,
            files=[f],          # <-- was: file=f  (wrong)
        )

    file_id = getattr(result, "file_id", None) or getattr(result, "id", None) or str(result)
    print(f"Uploaded memo: {ticker} {period_label} → {file_id}")
    return file_id
```
...
def verify_all_indexed(file_ids: list[str], poll_interval: int = 3, max_polls: int = 20) -> bool:
    """
    Poll /ingestion/verify_processing until all file_ids report status 'completed'.
    ...
    for attempt in range(max_polls):
        ...
        for fid in list(pending):
            resp = requests.get(
                ...
                headers=HEADERS,
                params={"file_ids": fid, "tenant_id": TENANT_ID},
            )
            data   = resp.json()
            status = data.get("status") or data.get("processing_status", "unknown")

            if status == "completed":
                pending.discard(fid)
                print(f"  ✓ {fid} - indexed")
            elif status == "failed":
                raise RuntimeError(f"Indexing failed for {fid}: {data}")
            else:
                print(f"  … {fid} - {status}")
...
verify_all_indexed(file_ids)
```

> **Batch limit reminder.** Maximum 20 sources per request. Wait 1 second between batches. Call `verify_processing` before any production query.

---

## Step 3 - Store Analyst Memory

Per-analyst memory personalizes recall based on the analyst's focus area, preferred companies, and communication style. A macro fund PM cares about different metrics than a sector-specialist equity analyst.
...
def store_analyst_profile(user_id: str, profile_text: str) -> dict:
    """
    Store an analyst's profile for personalized recall.
...
## Step 4 - Temporal Recall Queries
...
def query_point_in_time(
    ...
    """
    Answer questions about a specific quarter.
    ...
    mode: "thinking" - multi-query reranking, personalised recall.
    """
...
print(result["answer"])
# Output:
# Based on data for Q2 2023: Financial Metrics -- Q2 2023
#   revenue_usd: 9100000  arr_usd: 37200000  gross_margin_pct: 72.8
#   churn_rate_pct: 1.6   cac_usd: 4050      ltv_usd: 40500
#   burn_rate_usd: 980000 headcount: 156      ndr_pct: 115
# [Source: Memory | Relevance: 0.75]
# Chunks returned: 2
```
...
def query_trend(
    ...
    graph_ctx   = recall.get("graph_context", {})
    query_paths = graph_ctx.get("query_paths", [])
    ...
    chunks_sorted = sorted(
        chunks,
        key=lambda c: c.get("source_upload_time", "") or "",
    )
...
def cross_source_reconciliation(
    ...
    Uses three separate recall calls, one per sub-tenant, then merges context.
    """

    def recall(sub_tenant_id: str, doc_type_filter: str) -> list[dict]:
        resp = requests.post(
            f"{BASE_URL}/recall/full_recall",
            headers=HEADERS,
            json={
    ...
    transcript_chunks = recall(f"earnings-{ticker.lower()}", "earnings_transcript")
    metrics_chunks    = recall("internal-metrics",            "internal_metrics")
    memo_chunks       = recall("board-memos",                 "board_memo")
...
def track_narrative_shift(
    ...
    chunks = sorted(
        recall.get("chunks", []),
        key=lambda c: c.get("additional_metadata", {}).get("period_end_date", ""),
    )
...
## Step 5 - Q&A Search Interface

For direct Q&A (e.g. a chatbot interface for analysts), use `/search/qna`. It returns an AI-generated `answer` string directly, maintaining conversation context via `session_id`.

```python
# query/qna.py
import requests, uuid, os
from typing import Optional   # FIX: use Optional for Python 3.9 compatibility
...
# Per-analyst sessions - persist for the analyst's working session
...
def financial_qna(
    ...
    """
    General-purpose financial Q&A endpoint.

    Returns: {"answer": str, "chunks": [...], "session_id": str}

    The session_id maintains conversational context - a follow-up question
    ("and what about the previous quarter?") remembers the prior exchange.
    """
    if user_id not in analyst_sessions:
        analyst_sessions[user_id] = str(uuid.uuid4())

    # Build metadata filter from provided constraints
    filters: dict = {}
    ...
    payload: dict = {
        "question":      question,
        "session_id":    analyst_sessions[user_id],
        "tenant_id":     TENANT_ID,
        "sub_tenant_id": f"analyst-{user_id}",
        "user_name":     user_id,
        "max_results":   15,
    ...
    resp = requests.post(
        f"{BASE_URL}/search/qna", headers=HEADERS, json=payload
    )
    resp.raise_for_status()
    result = resp.json()
...
print(r1["answer"])
# Output: Based on data for Q4 2023: revenue_usd: 10,800,000  arr_usd: 43,500,000
#         gross_margin_pct: 73.1  churn_rate_pct: 1.9  burn_rate_usd: 890,000
#         [Source: Memory | Relevance: 0.75]
...
print(r2["answer"])
# Output: Q1 1.8% → Q2 1.6% (improvement) → Q3 2.1% (deterioration) → Q4 1.9% (recovery)
#         Chunks: 8 (all four quarters represented)
...
print(r3["answer"])
# Output: Yes - Q3 2023 churn_rate_pct: 2.1%, up from 1.6% in Q2 2023 (+0.5pp).
#         [Source: Internal Metrics Q3 2023]
...
print(r4["answer"])
# Output: Based on data for Q4 2023: ACME Q4 2023 Earnings Transcript...
#         [Source: ACME Q4 2023 Earnings Transcript | Relevance: 0.90]
```
...
def generate_quarterly_briefing(
    ...
    Full quarterly briefing pipeline:
      1. Recall this quarter's performance data (all sources)
      2. Recall prior quarters for trend context
    ...
    print("[1/4] Recalling current quarter...")
    current_q = requests.post(
        f"{BASE_URL}/recall/full_recall",
        headers=HEADERS,
        json={
            "tenant_id":     TENANT_ID,
    ...
    current_chunks = current_q.get("chunks", [])

    # ── 2. Prior quarters for trend context ──────────────────────────────
    print("[2/4] Recalling trend data...")
...
# FIX: import financial_qna with sys.path - avoids ModuleNotFoundError when
# running slack_bot.py directly from the integrations/ directory.
import sys
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from query.qna import financial_qna
...
curl -X POST 'https://api.hydradb.com/ingestion/upload_knowledge' \
  ...
  -F "file=@ACME_Q2_2023_transcript.pdf" \
...
```http
GET /ingestion/verify_processing?file_ids=FILE_ID&tenant_id=financial-analyst
```
...
{
  "question":      "What did management say about gross margin in Q2 2023?",
  "session_id":    "analyst-session-uuid",
  "tenant_id":     "financial-analyst",
...
### Trend Recall (raw chunks + graph paths)
...
### Response Shape - `/recall/full_recall`
...
| `file=f` instead of `files=[f]` in SDK | `TypeError` on `upload.knowledge()` | SDK expects a **list**: `files=[f]` not `file=f` |
| Raw `requests` for PDF upload | 422 error on `/ingestion/upload_knowledge` | Use the SDK: `pip install hydra-db-python` |
| No `verify_processing` call | Queries return empty results silently | Always verify before querying |
