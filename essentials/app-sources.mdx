...
| Field | Required? | What it does | Details |
|---|---|---|---|
| `kind` | Recommended | Tells HydraDB what type of app object this is. Valid values: `email`, `message`, `ticket`, `knowledge_base`, `custom`. | [Identity fields](#identity-fields), [Kind-specific fields](#kind-specific-fields) |
| `provider` | Recommended | Names the system the item came from, such as `slack`, `gmail`, `jira`, `notion`, or `salesforce`. | [Identity fields](#identity-fields) |
| `external_id` | Recommended | Stable provider ID for this exact item. | [Identity fields](#identity-fields) |
| `fields` | Yes | Structured content and app-specific fields for the selected `kind`. | [Kind-specific fields](#kind-specific-fields), [Thread and parent fields](#thread-and-parent-fields) |
| `metadata` | Optional | Flexible context about where the item lives, such as channel, project, space, workspace, or pipeline. | [Container metadata](#container-metadata) |
| `relations` | Optional | Explicit source-to-source connections, such as linked tickets, blockers, or CRM-to-support links. | [Relations](#relations) |
| `attachments` | Optional | Files attached to this app item. Include `content.text` or `content.markdown` when you already extracted attachment text. | [Attachments and comments](#attachments-and-comments) |
| `comments` | Optional | Comments or replies attached to this app item. | [Attachments and comments](#attachments-and-comments) |

For typed app sources, put the main searchable item text in `fields` (`email.body`, `message.body`, `ticket.description`, `knowledge_base.body`, or `custom.data`). The top-level `content` object is the older generic knowledge payload and is not required for app-native search.
...
| `kind` | Type of app object: `email`, `message`, `ticket`, `knowledge_base`, or `custom` | Helps HydraDB route queries like "Slack messages", "Jira tickets", or "wiki pages" |
...
|---|---|---|---|
| `thread_id` | Email, message, custom | The ID shared by all items in the same conversation or activity stream | Helps HydraDB pull nearby messages/items from the same thread when a query needs conversation context |
...
Attachments and comments are indexed separately from the main source text, but remain connected to the source.
...
| Relations | `reply_to`, `child_of`, `linked_to`, `blocks`, `related_to` traversal |
| `thread_id` | Same-thread expansion |
| `parent_id` | Parent/child hierarchy expansion |
| attachments/comments | Auxiliary vector search and shared attachment expansion |
...
This example ingests a Jira ticket with a parent epic, linked issues, project metadata, comments, and an explicit blocker relation.
...
| comments | Comment nodes, comment vectors, and author links |
...
| `kind` | `email`, `message`, `ticket`, `knowledge_base`, `custom` | Recommended | Tells HydraDB how to interpret `fields`. Defaults to `custom` if omitted. | Routes search by object type and selects the right parser for structured fields. |
...
| `comments` | List of comment objects | Optional | Adds comments/replies attached to the app item. | Makes comment text and comment authors searchable. |
...
| `properties` | Object | Optional | Extra attachment metadata. | Preserves provider-specific attachment context. |
...
| `properties` | Object | Optional | Extra comment metadata. | Preserves provider-specific comment context. |

### Relation fields

| Field | Type / allowed values | Required? | Why send it | Search / ingestion effect |
|---|---|---|---|---|
| `predicate` | String, e.g. `linked_to`, `blocks`, `child_of`, `reply_to`, `related_to` | Optional, defaults to `linked_to` | Names the relationship. | Lets search traverse only the relevant relationship type. |
