...
When you call [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-content) with `type=memory`, HydraDB turns your input into queryable user context.
...
The full schema lives on [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-content). For `type=memory`, send a JSON-stringified `memories` array; each item can use the fields below.
...
<Note>
The legacy alias `cortex_source_ids` is still accepted for backward compatibility. Prefer `ids` for new integrations.
</Note>
...
At query time, linked memories return in the `additional_context` field of the [query response](/api-reference/v2/endpoint/query). Forceful relation expansion is enabled by default for thinking-mode queries. Set `search_forceful_relations=false` to disable it.
...
<Warning>
**Memory items only.** Forceful relations work within the same store. Pointing a Memory item at a Knowledge source ID, or a Knowledge item at a Memory source ID, silently returns nothing at query time.
</Warning>

<Warning>
**`thinking` mode only.** Forceful-relation context is only fetched when `mode: "thinking"` is set on the quer request. Passing `search_forceful_relations: true` with `mode: "fast"` is rejected with HTTP `422` - set `mode: "thinking"` or remove the flag.
</Warning>
...
Poll [`GET /context/status`](/api-reference/v2/endpoint/sources-overview) with the returned `id` until `indexing_status` is `completed`. You can also use webhooks when available. After indexing, the memory can surface in [`POST /query`](/api-reference/v2/endpoint/query-overview) when called with `type: "memory"` or `type: "all"`.
...
| Storing shared docs as memories | They will not surface when querying `type: "knowledge"` | Use [knowledge ingestion](/essentials/v2/knowledge) with `type=knowledge` on [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-content) |
| Expecting immediate searchability | Ingestion is asynchronous | Poll [`GET /context/status`](/api-reference/v2/endpoint/sources-overview) until indexing is complete |
