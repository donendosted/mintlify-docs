...
4. **Make it searchable**  -  later, [`POST /query`](/api-reference/v2/endpoint/query) can retrieve it with `type: "memory"` or `type: "all"`.
...
| <Field name="tenant_metadata" type="string (JSON object)" /> | Tenant-schema fields for filtering and search. For memory ingestion, encode this nested object as a JSON string inside each `memories[]` item, for example `"tenant_metadata": "{\"department\":\"support\"}"`. |
...
| <Field name="document_metadata" type="object" /> | Deprecated alias for `additional_metadata`. Send this as an object. |
...
<Warning>
  **Memory metadata encoding:** `memories` is itself a JSON-stringified multipart field. Within each memory item, `tenant_metadata` is currently validated as a JSON-encoded string, while `additional_metadata` / `document_metadata` is validated as an object. If you pass `tenant_metadata` as an object, the API returns `400 INVALID_INPUT`.
</Warning>
...
- `type: "memory"` searches the Memories store (`vectorstore_status.memories`).
- `type: "knowledge"` searches the [Knowledge](/essentials/v2/knowledge) store (`vectorstore_status.knowledge`).
- `type: "all"` runs both in parallel and returns a single merged, re-ranked result set.

<Info>
  <strong>One endpoint, two stores.</strong>
  <p>For personalized responses grounded in shared context, use <code>type: "all"</code> on a single <a href="/api-reference/v2/endpoint/query"><code>POST /query</code></a> call. HydraDB runs both stores in parallel and merges results for you. See <a href="/essentials/v2/query">Search</a>.</p>
</Info>
...
At search time, linked memories return in the `additional_context` field of the [query response](/api-reference/v2/endpoint/query). Forceful relation expansion is enabled by default for thinking-mode searches. Set `search_forceful_relations=false` to disable it.
...
Poll [`GET /source/status`](/api-reference/v2/endpoint/sources-overview) with the returned `source_id` until `indexing_status` is `completed`. You can also use webhooks when available. After indexing, the memory can surface in [`POST /query`](/api-reference/v2/endpoint/query-overview) when called with `type: "memory"` or `type: "all"`.
...
| Storing shared docs as memories | They will not surface when searching `type: "knowledge"` | Use [knowledge ingestion](/essentials/v2/knowledge) with `type=knowledge` on [`POST /source/ingest`](/api-reference/v2/endpoint/ingest-content) |
