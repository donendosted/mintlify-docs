...
- `mode`  -  If omitted, defaults to `"fast"`. Pick `"fast"` or `"thinking"` explicitly when you know your traffic shape. Use `"auto"` when query complexity varies call-to-call: it scores the query before retrieval and routes to `"fast"` or `"thinking"` (defaulting to `"thinking"` when the signal is inconclusive), and overrides `graph_context` to match whichever it picks. `auto` must be requested explicitly - it never happens implicitly.
...
| `graph_context` value ignored | Request uses `mode: "auto"` | `auto` overrides `graph_context` to match whichever pipeline it routes to; set `graph_context` explicitly only under `mode: "fast"` or `"thinking"`. |
| Expected automatic routing, got `fast` every time | `mode` was omitted, not set to `"auto"` | Omitting `mode` defaults to `"fast"`  -  automatic routing only happens when you send `mode: "auto"` explicitly. |
