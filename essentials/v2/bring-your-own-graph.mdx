...
Bring Your Own Graph (BYOG) lets you attach a `graph_payload` — your own entities and relations — to a document on [`POST /context/ingest`](/api-reference/v2/endpoint/ingest-context). For that document, HydraDB **uses your graph instead of running LLM extraction**.

Your graph is stored in exactly the same shape extraction produces (`source → relation → target` triplets), so it answers queries identically — it shows up in the `graph_context` slice and its relations point back at the document's chunks. No query-side changes are needed.
...
| To supply the **full entity/relation graph for one document** | **Bring Your Own Graph** (this page) |

---

## 3. The `graph_payload` shape

`graph_payload` is a JSON string: a **map keyed by source id** — a document's `file_metadata` `id` or an `app_knowledge` item's `id` — where each value is that source's graph (an `entities` map + a `relations` list). Attach graphs to several sources in one request by adding more keys.
...
- **Top-level key** — the id of the source this graph belongs to (a document's `file_metadata` `id` or an `app_knowledge` item's `id`). A key matching no source in the request is rejected with `400`.
...
- **Chunk linking.** Each relation is linked to the document's most relevant chunk(s), so `graph_context` results hydrate the right passages. Linking is permissive (see [Limitations](#7-limitations)).
...
- **Durable across re-ingest.** Re-uploading the same source — or a connector re-sync, which carries no payload — re-applies your stored graph rather than re-extracting. Your facts are not lost on re-ingest.
...
<Note>
`graph_payload` is **per-source**: each top-level key must match a `file_metadata` document `id` or an `app_knowledge` item `id` in the same request. Attach graphs to multiple documents and/or app sources at once. A document must carry an `id` in `file_metadata` to be targeted (that `id` is the map key). Knowledge-only.
</Note>
...
curl -X POST 'https://api.hydradb.com/context/ingest' \
  ...
  -F 'app_knowledge=[{ "id": "slack-incident-42", "type": "slack", "content": { "text": "Platform team paged for the payments outage." } }]' \
  -F 'graph_payload={
    "billing-policy-doc": {
      "entities": {
...
with open("/path/to/billing-policy.pdf", "rb") as f1, open("/path/to/deploy-runbook.pdf", "rb") as f2:
    client.context.ingest(
        ...
        app_knowledge=json.dumps([
            {"id": "slack-incident-42", "type": "slack",
             "content": {"text": "Platform team paged for the payments outage."}},
        ]),
...
Your triplet comes back in `graph_context`, identical to an extracted one:
        ...
        "triplets": [
          {
            ...
            "relation": { "canonical_predicate": "OWNS", "context": "Alice Carter owns the billing policy." },
...
- **Replace, not augment.** A BYOG document has no LLM-extracted facts — only the graph you supply (plus normal chunk search). Augment mode is a future enhancement.
- **Permissive linking → possible false positives.** Every relation links to its best-matching chunk even if the match is weak; there is no reject floor yet. A linked relation is **sourced** (similar to a chunk), not necessarily **supported** (stated by the document).
- **Bulk, one-shot.** You supply the whole graph with the document. Per-triple add/update/delete is not yet available.
