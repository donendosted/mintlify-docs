...
For complete `tenant_metadata_schema` examples, supported field types, and advanced configuration options, see [Create Tenant API Reference](/api-reference/v2/endpoint/tenants-overview).
...
HydraDB exposes two metadata fields on every source object:

- **`metadata`**  -  schema-aligned fields intended for deterministic filtering. If a field is declared in the tenant metadata schema and match-enabled, it can be used in `metadata_filters`.
- **`additional_metadata`**  -  free-form fields for display, bookkeeping, or downstream application use. Do not rely on these for deterministic filtering.

If a field needs to be filterable at recall time, put it in `metadata` and plan it in the tenant metadata schema. Otherwise, prefer `additional_metadata`.

### `tenant_metadata` vs `document_metadata`

In the v2 API, the two metadata buckets on every source are exposed under the names `tenant_metadata` and `document_metadata`:

- **`tenant_metadata`**  -  matches the schema you defined at tenant creation (`tenant_metadata_schema`). Fields are indexed and filterable tenant-wide. Use for stable, reusable attributes that apply across many documents: `department`, `region`, `customer_tier`, `compliance_framework`.
- **`document_metadata`**  -  free-form per-document JSON. Flexible, but to filter on it you must opt in by nesting under a `document_metadata` key in `metadata_filters`:
...
**Rule of thumb:** structured + reusable across documents → `tenant_metadata`; ad-hoc or unique to one document → `document_metadata`.
...
The exact tenant creation request may include additional fields depending on your setup. See the [Create Tenant API Reference](/api-reference/v2/endpoint/tenants-overview) for the complete schema.
  ...
  "app_knowledge": [
    {
      ...
      "content": { "text": "Authentication controls are reviewed quarterly for SOC2." },
      "metadata": {
        "compliance_framework": "SOC2"
      }
    }
...
- [Recall](/essentials/v2/recall) - using `metadata_filters` during retrieval
- [Memories](/essentials/v2/memories) - metadata fields on memory items
- [Multi-Tenant Support](/essentials/v2/multi-tenant) - tenant and sub-tenant scoping
- [Create Tenant API Reference](/api-reference/v2/endpoint/tenants-overview) - defining tenant metadata schema
- [Full Recall API Reference](/api-reference/v2/endpoint/search-overview) - using metadata filters in recall
