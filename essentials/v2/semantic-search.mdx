...
`POST /search` is the single recall endpoint. Two parameters decide what runs:

- **`source`** picks the collection: `"sources"` (Knowledge), `"memories"` (user-scoped Memories), or `"all"` (both, merged and re-ranked together).
...
`alpha` controls the semantic-vs-lexical blend when `search_by: "hybrid"`. Higher values lean semantic. Lower values lean lexical.
...
- [Recall](/essentials/v2/recall) - full parameter reference and parallel recall patterns
