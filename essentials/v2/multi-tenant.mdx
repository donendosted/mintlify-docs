...
- `source: "sources"` retrieves shared Knowledge (Knowledge vector store, `vectorstore_status` index `1`).
...
When you need different formatting for shared vs personal context in the LLM prompt, call `POST /search` twice (once with `source: "sources"`, once with `source: "memories"`) and combine in your application.
...
- `source: "sources"` retrieves **Knowledge** (shared documents and app sources).
