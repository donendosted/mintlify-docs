...
- **`meta.deprecation`** — a human-readable string in the response envelope's `meta` object (for endpoints that return the envelope). Fully-migrated requests (canonical route + canonical fields) omit it.
...
```json
{
  ...
  "meta": {
    ...
    "latency_ms": 12.3,
    "deprecation": "The /tenants routes are deprecated. Migrate to the /databases routes."
  }
}
```

Use these signals to find and retire legacy usage; once you send only `database`/`collection` on the `/databases` routes, the header and `meta.deprecation` disappear.

### Sending both names

If you send **both** a canonical field and its deprecated alias:

- **Same value** (for example `database` and `tenant_id` both `"acme"`) — accepted; HydraDB uses the `database`/`collection` value.
- **Different values** (for example `database: "acme"` and `tenant_id: "other"`) — rejected with `400`, since the two names refer to the same thing and must agree. Send only one.
