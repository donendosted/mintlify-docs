...
|---|---|---|
| **Control** | API authentication, tenant lifecycle, provisioning, and status | [`/databases`](/api-reference/v2/endpoint/tenants-overview) family |
...
1. **Create a tenant** with [`POST /databases`](/api-reference/v2/endpoint/create-tenant)  -  your isolated workspace, optionally with a [metadata schema](/essentials/v2/metadata) declared up front.
...
| `collection` | Ingest + query | Narrows data to a user, team, or workspace inside the database. |
