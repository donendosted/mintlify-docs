...
<Info>
Webhooks are sent for terminal indexing states. For progress updates before completion, use [Verify Processing](/api-reference/v2/endpoint/sources-overview).
</Info>
...
async def hydradb_webhook(
    request: Request,
    x_cortex_delivery_id: str | None = Header(default=None),
    x_cortex_signature: str | None = Header(default=None),
):
    ...
    if SIGNING_SECRET:
        ...
        if not x_cortex_signature or not hmac.compare_digest(x_cortex_signature, expected):
            return Response(status_code=401)

    event = json.loads(raw_body)

    # Store x_cortex_delivery_id and skip it if you have already processed it.
    print("HydraDB webhook", x_cortex_delivery_id, event["id"], event["status"])
