...
| **Sent in** | `app_knowledge` as a stringified JSON object or array | `files` plus optional `document_metadata` |
...
| Deleted ticket comment | Call [`DELETE /source`](/api-reference/v2/endpoint/delete-source) with the comment source `id` | The comment should no longer surface. |
...
For a reply, ingest a second source. `fields.parent_id` points to the parent message's provider `external_id`; an explicit relation can point to either the parent source ID or the provider ID.
...
|---|---|
| `{ "id": "..." }` | You already know the HydraDB source ID. |
...
|---|---|---|
| <Field name="id" type="string" required /> | Stable HydraDB source ID for this item. | Upsert key, delete/status/list ID, and direct relation target. |
...
| <Field name="target.id" type="string" /> | Direct HydraDB source ID target. | Connects to a known source. |
