...
| The key is declared in `tenant_metadata_schema` with `enable_match: true` | The key is unique or not worth adding to the database schema |
...
Filterable database metadata keys must be declared in the database schema. Query-time filters use top-level keys for `metadata` and nested `additional_metadata` for free-form fields:
