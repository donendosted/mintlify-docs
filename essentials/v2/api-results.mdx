...
description: "Turn a search response into a grounded LLM answer."
---

A search response is structured JSON. Your model wants prose. This page shows how to bridge that gap  -  turning a `RetrievalResult` into a clean, grounded prompt for an LLM.

The same pattern works for every call to [`POST /search`](/api-reference/v2/endpoint/search-overview), whether you set `source: "knowledge"`, `source: "memories"`, or `source: "all"`  -  the response shape is identical.
...
First, call `POST /search` with the parameters you need:
...
# returned by POST /search into a formatted context string.
#
# The logic remains the same, except that search objects are now dictionaries
...
def build_context_string(result) -> str:
    """
    Build a formatted context string from a HydraDB /search response.
...
For personalised answers, the simplest path is one `POST /search` with `source: "all"`  -  HydraDB searches both stores in parallel and returns one merged, ranked result set.
...
If you need to format Knowledge and Memories differently (e.g., label "User preferences:" vs "Relevant docs:" sections), call `POST /search` twice in parallel with `source: "knowledge"` and `source: "memories"`, format each result, and combine them into one prompt. There's no required ordering  -  preferences before docs is purely for readability.

```python
import asyncio

knowledge_result, memory_result = await asyncio.gather(
    async_client.search.query(
        tenant_id="acme_corp", query="refund policy",
        source="knowledge", search_by="hybrid", mode="thinking",
    ),
    async_client.search.query(
        tenant_id="acme_corp", sub_tenant_id="user_123",
        query="answer style preferences",
        source="memories", search_by="hybrid",
    ),
)
...
| Ignoring Memories when personalisation matters | Answers feel generic | Use `source: "all"` on `POST /search` (or call `source: "memories"` separately) and include the result in the prompt. |
...
| Expecting graph fields without requesting them | Graph context will not be populated | Set `graph_context: true` on the search call. |
