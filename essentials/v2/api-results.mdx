...
The same pattern works for every call to [`POST /search`](/api-reference/v2/endpoint/search-overview), whether you set `source: "sources"`, `source: "memories"`, or `source: "all"`  -  the response shape is identical.
...
knowledge_result, memory_result = await asyncio.gather(
    async_client.search.query(
        ...
        source="sources", search_by="hybrid", mode="thinking",
    ),
    async_client.search.query(
...
- [Recall](/essentials/v2/recall)  -  request parameters and response shape
- [Memories](/essentials/v2/memories)  -  what `POST /search` with `source: "memories"` searches
- [Knowledge](/essentials/v2/knowledge)  -  what `POST /search` with `source: "sources"` searches
