...
Connector schemas are provider-owned and available through the API for every supported connector. Pass the provider identifier (from `GET /connector-catalog`) as the `id` query parameter:

```bash
curl 'https://api.hydradb.com/connectors/providers?id=linear' \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
  -H "API-Version: 2"
```

The response returns the provider identity, the source streams HydraDB turns into app sources (`source_object_types`), the streams it loads only to enrich them (`enrichment_object_types`), and every normalized field it stores. Metadata is not a separate list — each entry in `fields` carries a `location` that says where the value lives:

```json
{
  "provider": "linear",
  "connector_type": "linear",
  "source_object_types": ["Issues"],
  "enrichment_object_types": [],
  "fields": [
    {
      "name": "title",
      "data_type": "string",
    ...
    },
    {
      ...
      "filterable": false,
      "searchable": true,
      "description": "Primary text indexed for semantic and full-text search."
    },
    {
      "name": "linear_identifier",
      "data_type": "string",
      "location": "additional_metadata",
      "filterable": true,
      "searchable": false,
...
| `location` | Where the value lives: `top_level` (identity fields like `title` and `timestamp`), `fields` (app content), or `additional_metadata` (exact-match filter keys) |
...
For Gmail, use `account_email` (a field with `location: "additional_metadata"`) rather than `provider_account_scope` when filtering to a connected account. The Gmail provider response calls out this exception in its field description.
