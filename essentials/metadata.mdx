...
**Note:** When ingesting, use `metadata` (tenant-level) and `additional_metadata` (document-level) in the request body. In recall responses, these are returned as `tenant_metadata` and `document_metadata` respectively.

---

## Two-Tier Metadata Architecture

HydraDB supports two tiers of metadata for enterprise-scale systems.
...
```json
{
  "tenant_metadata_schema": [
    {"name": "compliance_framework", "data_type": "VARCHAR", "enable_match": true, "enable_dense_embedding": true},
    {"name": "data_classification", "data_type": "VARCHAR", "enable_match": true},
...
The filter is applied _before_ semantic and graph ranking — only matching documents enter the retrieval pipeline.
