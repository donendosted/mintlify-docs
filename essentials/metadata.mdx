...
HydraDB exposes two common metadata patterns:
...
Some endpoints accept metadata as objects; memory item fields such as `tenant_metadata` and `document_metadata` are JSON strings. Check the relevant endpoint reference before sending metadata.
