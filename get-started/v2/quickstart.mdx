...
Poll [`GET /tenants/status`](/api-reference/v2/endpoint/tenant-status) until  `vectorstore_status` entries and `graph_status` are `true`. The snippets below loop automatically:
